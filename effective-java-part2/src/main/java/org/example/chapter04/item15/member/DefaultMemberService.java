...
class DefaultMemberService implements MemberService{ // MemberService의 구현체이기 때문에 내부 패키지 안에서만 존재해도 됨
    ...
    // 외부 클래스였던 것이 독립적인 내부클래스로 변경됨. 이 클래스에 Comparator를 implement한다면?
    private static class PrivateStaticClass implements Comparator<DefaultMemberService> {


        @Override
        public int compare(DefaultMemberService o1, DefaultMemberService o2) {
            return 0;
        }
    }
